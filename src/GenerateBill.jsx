...
function GenerateBill({ onNavigate }) {
    ...
    try {
      ...
      if (details.length > 0) {
        ...
        const billItems = Object.values(groupedItems).map((group) => {
          ...
          // Sort DC numbers naturally (create a copy to avoid mutating original)
          const sortedDCNumbers = (group.dcNumbers || []).slice().sort((a, b) => {
  ...
  const handleSave = async () => {
    ...
    if (!formData.invoiceNumber || !formData.customerName) {
      alert("Please ensure Invoice Number and Customer Name are filled");
      return;
    }
    ...
    // Set filename for the PDF - remove all spaces
    const originalTitle = document.title;
    const cleanInvoiceNumber = formData.invoiceNumber.trim().replace(/\s+/g, '');
    const cleanCustomerName = formData.customerName.trim().replace(/\s+/g, '');
    const fileName = `${cleanInvoiceNumber}-${cleanCustomerName}`;
    document.title = fileName;
    
    // Open print dialog (user can choose "Save as PDF" and select location)
    window.print();
          ...
          <div className="bill-modal-wrapper">
            <div className="bill-modal-content">
              ...
              <div className="bill-actions no-print">
                ...
                <button onClick={handleSave} className="btn-save">💾 Save as PDF</button>
              </div>
